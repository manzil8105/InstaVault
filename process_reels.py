...
# schemas, you can edit out your own categories 
...
def download_reel_stateless(url: str, post_id: str) -> str:
    """Downloads the MP4 directly using stateless scraping (no cookies)."""
    out_path = TEMP_DIR / f"{post_id}.mp4"
    
    print(f"  -> Downloading MP4 for {post_id}...")
    cmd = [
        "yt-dlp",
        url,
        "-f", "b",
    ...
    if not out_path.exists():
        raise Exception(f"Failed to fetch video. It may be private or deleted.\nLog: {result.stderr.strip()}")
...
def process_link(url: str):
    ...
    try:
        # download
        video_path = download_reel_stateless(clean_url, post_id)
        
        # upload to Gemini
        print("  -> Uploading to Gemini API...")
        uploaded_file = client.files.upload(file=video_path)
        
        # wait for Google's servers to process the video
            ...
            try:
                response = client.models.generate_content(
                    ...
                    config=types.GenerateContentConfig(
                        ...
                        temperature=0.0,
                        # Suppresses the AFC terminal warning
...
def main():
    ...
    print("  instagram reel > obsidian pipeline   ")
    
    
    while True:
        try:
            user_input = input("\nPaste Instagram link separated by spaces (or type 'exit'): ").strip()
            ...
            links = [link for link in user_input.split() if "instagram.com" in link]
            
            if not links:
                print("No valid Instagram links detected.")
                continue
